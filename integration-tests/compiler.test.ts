...
describe('Compiler Integration Tests', () => {
    ...
    test('should parse a simple variable declaration and build a correct AST', () => {
        ...
        // The grammar is imported from a shared file to be reusable
        const parser = new SLRParser(fullGrammar);
        const ast = parser.parse(tokens);

        // 1. Check if the parser returns a Program
    ...
    test('should handle an if-else statement with expressions', () => {
        ...
        expect(condition.left).toEqual(new Identifier('x'));
        expect(condition.right).toEqual(new Literal(10));
        ...
        expect(thenDecl.initializer).toEqual(new Literal(1));
        ...
        expect(elseDecl.initializer).toEqual(new Literal(2));
    });
    ...
    test('should correctly parse operator precedence in expressions', () => {
        ...
        expect(addExpr.left).toEqual(new Literal(10));

        expect(addExpr.right).toBeInstanceOf(BinaryExpr);
        const mulExpr = addExpr.right as BinaryExpr;
        expect(mulExpr.operator).toBe('*');
        expect(mulExpr.left).toEqual(new Literal(2));
        expect(mulExpr.right).toEqual(new Literal(5));
    });
    ...
    test('should handle a while statement', () => {
        ...
        expect(condition.operator).toBe('<');
        expect(condition.left).toEqual(new Identifier('i'));
        expect(condition.right).toEqual(new Literal(100));
    ...
    test('should handle a function call with arguments', () => {
        ...
        expect(callExpr.args[0]).toEqual(new Literal(42));
        expect(callExpr.args[1]).toEqual(new Literal('hello'));
    });
    ...
    test('should handle array declaration and literals', () => {
        ...
        expect(arrayLiteral.elements[0]).toEqual(new Literal(1));
        expect(arrayLiteral.elements[1]).toEqual(new Literal(2));
        expect(arrayLiteral.elements[2]).toEqual(new Literal(3));
    });
    ...
    test('should handle array access', () => {
        ...
        expect(arrayAccess.array).toEqual(new Identifier('myArray'));
        expect(arrayAccess.index).toEqual(new Literal(0));
    });
}); 